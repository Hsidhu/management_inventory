                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('receipts.product.update', ['receipt' => $receipt, 'receivedproduct' => $receivedproduct]) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('product_id') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-stock">Price</label>
                                    <input type="number" name="price" id="input-stock" class="form-control form-control-alternative{{ $errors->has('product_id') ? ' is-invalid' : '' }}" value="{{ old('price', $receivedproduct->price) }}" required>
                                    @include('alerts.feedback', ['field' => 'product_id'])
                                </div>
